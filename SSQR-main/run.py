...
class Runner(object):
    def __init__(self, params):
        ...
        wandb.init(project="VQKG_1015", name='VQ4KG',config=params)
        ...
        # ent_emd 3072 40943
        # self.ent_text_emd = np.load('../MyKGVQ/data/wn18rr/ent_emd.npy')
    ...
    def gen_ent_codes(self):
        ...
        save_name = f'./codes_new/{dataset}_{self.p.seq_len}_{self.p.num_code}_notext.pt'  # GCN 2  _notext _nogcn
    ...
    def gen_ent_codes_emds(self):
        ...
        dataset = self.p.dataset.split('/')[-1]
        save_name = f'./codes_new/{dataset}_{self.p.seq_len}_{self.p.num_code}_emd_notext.pt'  # GCN 2
    ...
    def train(self):  # 所有整体训练
        ...
        for step, (triplets, labels) in enumerate(train_iter):
            ...
            pred, q_loss, x_loss = self.model(self.g, subj, rel, self.ent_text_emd, stage=3)  # [batch_size, num_ent]  ent_text
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', dest='dataset', default='data/FB15k-237', help='Dataset to use, default: FB15k-237')  # FB15k-237  wn18rr
